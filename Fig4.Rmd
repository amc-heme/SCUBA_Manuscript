...
# fetch_data method performance comparison for one feature

The testing below replicates the results shown in **Figure 4A** of the manuscript. Data for one feature was retrieved from each of the subsets using fetch_data, and the run time was measured via `sys.time`. For Seurat objects, SeuratObject::FetchData is performed as well as SCUBA::fetch_data, and the performance of each is compared. SCUBA::fetch_data is then ran on identical SingleCellExperiment and anndata objects to evaluate the performance of the new fetch_data methods for these object types.
...
for (subset_size in subset_sizes){
    ...
    for (i in trials){
        ...
        # Store the function name for the trial
        ...
        n_cells <- c(n_cells, subset_size)
        # Store the function name for the trial
        fxn <- c(fxn, "fetch_data.Seurat")
        
        # Display for subset size: add carrage return before next subset,
        # to avoid cluttering output during testing
        cat("\r")
    }
}
...
for (subset_size in subset_sizes){
    ...
    for (i in trials){
        ...
        # Run fetch_data and time run with Sys.time()
        start_time <- Sys.time()
...
sce_trial$fxn <- "fetch_data.SingleCellExperiment"
```
...
anndata_trial$fxn <- "fetch_data.AnnDataR6"
...
write.csv(
  fetchdata_results, 
  file = paste0(working_dir, "performance_results/fetch_data.csv"), 
  row.names = FALSE
)
```

# fetch_data vs. fetch_metadata for one metadata variable

The testing below replicates the results shown in **Figure 4A** of the manuscript. The script compares the run time of fetching one metadata feature using `fetch_data` compared to using the `fetch_metadata` convenience function provided by SCUBA.

```{r}
subset_sizes <- c(1000, 2000, 5000, 10000, 20000, 50000, 100000, 200000, 500000)
trials <- 1:5
...
for (subset_size in subset_sizes){
    ...
    for (i in trials){
        ...
        # and the function ran 
        n_cells <- c(n_cells, subset_size)
        fxn <- c(fxn, "fetch_data.SingleCellExperiment")
...
for (subset_size in subset_sizes){
    ...
    for (i in trials){
        ...
        # and the function ran 
        n_cells <- c(n_cells, subset_size)
        fxn <- c(fxn, "fetch_data.AnnDataR6")
...
```{r}
fetch_data_vs_fetch_metadata_results <- 
  rbind(
    seurat_trial, 
    sce_trial, 
    anndata_trial
...
write.csv(
  ...
  file = paste0(
    working_dir,
    "performance_results/fetch_data_vs_fetch_metadata.csv"
  ),
  row.names = FALSE
)
```

# fetch_metadata vs. fetch_reduction for reduction coordinates

The testing below replicates the results shown in **Figure 4B** of the manuscript. The script compares the run time of fetching reduction coordinates for one dimensional reduction using `fetch_data` compared to using the `fetch_reduction` convenience function provided by SCUBA.

```{r}
# fetch_data vs. fetch_reduction
# Seurat
times <- c()
n_cells <- c()
fxn <- c()
...
write.csv(
   ...
   file = paste0(
     working_dir,
     "performance_results/fetch_data_vs_fetch_reduction.csv"
   ),
   row.names = FALSE
)
```
...
fetch_data_path <- 
  paste0(
     working_dir,
     "performance_results/fetch_data.csv"
   )

fetch_data_vs_fetch_metadata_path <-
  paste0(
     working_dir,
     "performance_results/fetch_data_vs_fetch_metadata.csv"
   )

fetch_data_vs_fetch_reduction_path <-
  paste0(
     working_dir,
     "performance_results/fetch_data_vs_fetch_reduction.csv"
   )
...
```{r fetchData plot}
results_df <- read.csv(fetch_data_path)

# Add information on the generic used to run the tests
results_df$generic <-
  case_when(
    results_df$fxn == "FetchData.Seurat" ~ "SeuratObject::FetchData",
    results_df$fxn %in% 
      c("fetch_data.Seurat", 
        "fetch_data.SingleCellExperiment",
        "fetch_data.AnnDataR6") ~ "SCUBA::fetch_data"
    )

# Rename function column levels
results_df$fxn <- 
  plyr::revalue(
    results_df$fxn, 
    c("FetchData.Seurat" = "SeuratObject::FetchData",
      "fetch_data.Seurat" = "Seurat", 
      "fetch_data.SingleCellExperiment" = "SingleCellExperiment", 
      "fetch_data.AnnDataR6" = "anndata"
      )
    )
...
fetchData_comparison <- 
  ...
  scale_color_manual(
    name = NULL,
    # 7: black
    # 1: blue (color for Seurat in user guide vignette)
    # 3: green (SingleCellExperiment in user guide vignette)
    # 2: orange (anndata in user guide vignette)
    values = scuba_manuscript_pal[c(7,1,3,2)],
    aesthetics = "color",
    na.value = "grey50"
    ) +
  # labs(
  #   title = "fetch_data with a Single Gene", 
...
results_df$object <- 
  case_when(
    results_df$fxn %in% 
      c("fetch_data.Seurat", "fetch_metadata.Seurat") ~ "Seurat",
    results_df$fxn %in% 
      c("fetch_data.SingleCellExperiment", 
        "fetch_metadata.SingleCellExperiment") ~ "SingleCellExperiment",
    results_df$fxn %in%
      c("fetch_data.AnnDataR6", "fetch_metadata.AnnDataR6") ~ "anndata",
    TRUE ~ NA
  )
...
results_df$Function <- 
  case_when(
    results_df$fxn %in% 
      c("fetch_data.Seurat", 
        "fetch_data.SingleCellExperiment", 
        "fetch_data.AnnDataR6") ~ "fetch_data",
    results_df$fxn %in% 
...
results_df$Function <-
  factor(
    results_df$Function,
    levels = c("fetch_data", "fetch_metadata")
  )
...
# Remove single outlier for SingleCellExperiment, fetch_data, 10,000 cells
...
fetch_data_vs_fetch_metadata <-
  ...
  # labs(
  #   title = "fetch_data vs. fetch_metadata", 
  #   #subtitle = "Assessed over 100 trials, on full AML dataset, with one feature"
  #   ) +
  ylab("Run Time (s)") +
  xlab("Number of Cells") +
...
```{r fetch_data vs. fetch_reduction boxplot}
results_df <- 
...
results_df$object <- 
  case_when(
    results_df$fxn %in% 
      c("fetch_data.Seurat", "fetch_reduction.Seurat") ~ "Seurat",
    results_df$fxn %in% 
      c("fetch_data.SingleCellExperiment", 
        "fetch_reduction.SingleCellExperiment") ~ "SingleCellExperiment",
    results_df$fxn %in%
      c("fetch_data.AnnDataR6", "fetch_reduction.AnnDataR6") ~ "anndata",
    TRUE ~ NA
  )
...
results_df$Function <-
  factor(
    results_df$Function,
    levels = c("fetch_data", "fetch_reduction")
  )
